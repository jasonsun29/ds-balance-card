...
async function fetchMoonshot(ctx, key) {
  const body = await curlJson(ctx, 'https://api.moonshot.ai/v1/users/me/balance', key)
...
const PLATFORMS = [
  ...
  { id: 'moonshot', name: 'Moonshot Kimi', credentials: ['MOONSHOT_API_KEY'], fetch: fetchMoonshot },
  { id: 'stepfun', name: '阶跃星辰', credentials: ['STEPFUN_API_KEY'], fetch: fetchStepFun },
  { id: 'minimax', name: 'MiniMax', credentials: ['MINIMAX_API_KEY'], fetch: fetchMiniMax },
...
export function apply(ctx) {
  ctx.connection.rpc.handle('/dsbalance', async (endpoint, _payload, _signal) => {
    try {
      if (endpoint === 'fetch-all') return await fetchAll(ctx)

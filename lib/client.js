window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = '.dsbc-card{position:fixed;right:20px;bottom:20px;z-index:400;width:272px;box-sizing:border-box;pointer-events:auto;user-select:none;font-family:system-ui,-apple-system,"Segoe UI",Roboto,sans-serif;font-size:14px;line-height:1.5;color:#e5e7eb;background:rgba(15,23,42,.94);border:1px solid rgba(148,163,184,.28);border-radius:12px;box-shadow:0 10px 28px rgba(0,0,0,.38);backdrop-filter:blur(10px);overflow:hidden}.dsbc-header{display:flex;align-items:center;gap:8px;padding:8px 10px;cursor:grab;touch-action:none}.dsbc-header:active{cursor:grabbing}.dsbc-dot{width:8px;height:8px;border-radius:50%;flex:none}.dsbc-pulse{animation:dsbc-pulse 2s ease-in-out infinite}.dsbc-title{flex:1;font-weight:600;font-size:13.5px;letter-spacing:.02em}.dsbc-btn{border:none;background:transparent;color:#94a3b8;cursor:pointer;font-size:15px;line-height:1;padding:4px 5px;border-radius:6px}.dsbc-btn:hover{background:rgba(148,163,184,.18);color:#f1f5f9}.dsbc-body{padding:2px 12px 12px;max-height:380px;overflow-y:auto}.dsbc-section{margin:8px 0 2px;font-size:11.5px;font-weight:600;letter-spacing:.08em;color:#64748b}.dsbc-bars{margin:4px 0 0 12px;display:flex;flex-direction:column;gap:5px}.dsbc-bar-row{display:flex;align-items:center;gap:6px}.dsbc-bar-label{font-size:12px;color:#94a3b8;min-width:18px}.dsbc-bar{flex:1;height:7px;border-radius:3.5px;background:rgba(148,163,184,.16);overflow:hidden}.dsbc-bar-fill{height:100%;border-radius:3.5px}.dsbc-bar-text{font-size:12px;color:#cbd5e1;min-width:70px;text-align:right;font-variant-numeric:tabular-nums}.dsbc-plat{padding:7px 0;border-bottom:1px solid rgba(148,163,184,.12)}.dsbc-plat:last-child{border-bottom:none}.dsbc-plat-top{display:flex;align-items:center;gap:6px}.dsbc-plat-dot{width:6px;height:6px;border-radius:50%;flex:none}.dsbc-plat-name{flex:1;font-weight:600;font-size:13.5px;color:#cbd5e1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.dsbc-plat-main{font-weight:700;font-size:15px;font-variant-numeric:tabular-nums;color:#e5e7eb}.dsbc-plat-sub{margin:2px 0 0 12px;font-size:12px;color:#64748b;word-break:break-all}.dsbc-plat-sub b{color:#94a3b8;font-weight:600}.dsbc-note{margin-top:8px;padding-top:6px;border-top:1px solid rgba(148,163,184,.16);font-size:12px;color:#64748b}.dsbc-error{color:#fca5a5;font-size:13px;word-break:break-all}.dsbc-settings{display:flex;align-items:center;gap:6px;margin-top:8px;font-size:12.5px;color:#94a3b8}.dsbc-input{width:64px;background:rgba(148,163,184,.12);border:1px solid rgba(148,163,184,.3);border-radius:6px;color:#e5e7eb;padding:3px 6px;font-size:13px}@keyframes dsbc-pulse{0%,100%{opacity:1}50%{opacity:.3}}'
    ...
    function apply(ctx) {
      ...
      function BalanceCard() {
        ...
        const platformRow = (p, mode) => {
          const lv = platformLevel(p, threshold)
          let main = null
          const subs = []
          if (mode === "plan" && p.plan) {
            const rows = p.plan.rows || []
            const usedFor = (suffix) => {
              const matched = rows.filter((r) => typeof r.remainingPct === "number" && String(r.label).indexOf(suffix) !== -1)
              if (matched.length === 0) return null
              return Math.max(0, Math.min(100, Math.max(...matched.map((r) => 100 - r.remainingPct))))
            }
            const bar = (key, label, used) => {
              const color = used === null ? "#64748b" : used >= 100 ? "#f87171" : used >= 80 ? "#fbbf24" : "#34d399"
              return React.createElement("div", { key: key, className: "dsbc-bar-row" },
                React.createElement("span", { className: "dsbc-bar-label" }, label),
                React.createElement("div", { className: "dsbc-bar" },
                  React.createElement("div", { className: "dsbc-bar-fill", style: { width: String(used === null ? 0 : used) + "%", background: color } })),
                React.createElement("span", { className: "dsbc-bar-text" }, used === null ? "\u2014" : "已用 " + fmtPct(used) + "%"))
            }
            const extraTexts = rows
              .filter((r) => typeof r.remainingPct === "number" && String(r.label).indexOf("5h") === -1 && String(r.label).indexOf("\u5468") === -1)
              .map((r) => r.label + " 剩余 " + fmtPct(r.remainingPct) + "%")
            const planSubs = []
            if (extraTexts.length > 0) planSubs.push(extraTexts.join(" \u00B7 "))
            return React.createElement("div", { key: p.id, className: "dsbc-plat" },
              ...
              React.createElement("div", { className: "dsbc-bars" },
                bar("b5h", "5h", usedFor("5h")),
                bar("bweek", "\u5468", usedFor("\u5468"))),
              planSubs.length > 0 ? React.createElement("div", { className: "dsbc-plat-sub" }, planSubs.map((text, i) => React.createElement("div", { key: i }, text))) : null)
          }
          if (mode === "other" && p.supported === false) {
            main = React.createElement("span", { style: { color: "#94a3b8", fontWeight: 400 } }, "暂不支持查询")
            subs.push("该平台暂不支持 API 查询")
          } else if (mode === "other" && p.error) {
            main = React.createElement("span", { style: { color: "#fca5a5", fontWeight: 400 } }, "查询失败")
            subs.push(String(p.error).slice(0, 140))
          } else if (mode === "balance" && p.balance) {
            const symbol = symbolOf(p.balance.currency)
            const valueText = typeof p.balance.value === "number" ? symbol + fmtMoney(p.balance.value) : "\u2014"
            main = React.createElement("span", null, valueText)
            const parts = []
            for (const sub of p.balance.sub || []) {
              if (sub.value !== null && sub.value !== undefined && String(sub.value) !== "") parts.push(sub.label + " " + symbol + fmtMoney(sub.value))
            }
        ...
        if (!minimized) {
          if (state.status === "loading") {
            ...
            const planItems = state.platforms.filter((p) => p.supported !== false && !p.error && p.plan)
            const balanceItems = state.platforms.filter((p) => p.supported !== false && !p.error && p.balance)
            const otherItems = state.platforms.filter((p) => !(p.supported !== false && !p.error && (p.plan || p.balance)))
            const section = (key, title) => React.createElement("div", { key: key, className: "dsbc-section" }, title)
            const children = []
            if (planItems.length > 0) {
              children.push(section("sec-plan", "Token Plan / 套餐额度"))
              for (const p of planItems) children.push(platformRow(p, "plan"))
            }
            if (balanceItems.length > 0) {
              children.push(section("sec-balance", "余额"))
              for (const p of balanceItems) children.push(platformRow(p, "balance"))
            }
            for (const p of otherItems) children.push(platformRow(p, "other"))
            body = React.createElement("div", { className: "dsbc-body" },
              children,
        ...
        const style = { width: minimized ? "auto" : 272, pointerEvents: "auto", borderColor: borderColor }
        if (pos !== null) {
          style.left = pos.x
          style.top = pos.y
          style.right = "auto"

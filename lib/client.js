    ...
    function apply(ctx) {
      ...
      function BalanceCard() {
        ...
        const platformRow = (p, mode) => {
          ...
          } else if (mode === "plan" && p.plan) {
            const first = (p.plan.rows || []).find((r) => typeof r.remainingPct === "number")
            main = React.createElement("span", null, first !== undefined ? "剩余 " + fmtPct(first.remainingPct) + "%" : "\u2014")
            const rowTexts = (p.plan.rows || []).map((r) => {
              const pctText = typeof r.remainingPct === "number" ? fmtPct(r.remainingPct) + "%" : "\u2014"
